...
# -*- coding: utf-8 -*-
'''
MqttApp.py (version 0.2)
version 0.2: 
- add wlan0 IP address detection
- fix self._config non-iterable after config file read issue
...
class MqttApp():
	...
	def __init__(self, argv, client, debugging_mqtt=False):
		...
		self._ipaddress = os.popen("ifconfig eth0 \
			| grep 'inet ' \
			| awk '{print $2}' \
			| awk '{print $1}'").read().rstrip('\r\n')

		if not self._ipaddress :
			self._ipaddress = os.popen("ifconfig wlan0 \
				| grep 'inet ' \
				| awk '{print $2}' \
				| awk '{print $1}'").read().rstrip('\r\n')
	...
	def start(self):	
		...
		if self._mqttOutbox:
			try:
				self._mqttClient.will_set(self._mqttOutbox, payload="DISCONNECTED FROM {}".format(self._ipaddress), qos=1, retain=True)
			except Exception as e:
